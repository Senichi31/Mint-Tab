document.addEventListener('DOMContentLoaded', function () {
  ...
  const sidebarToggle = document.getElementById('sidebar-toggle-inner');
  ...
  function renderContent(items, isSearch = false) {
    ...
    displayItems.forEach(child => {
        ...
        item.onclick = (e) => {
            ...
            chrome.tabs.update({ url: child.url }); // 在当前活动的标签页更新 URL
            //window.close(); // 跳转后自动关闭插件的小窗口
        };    

      const icon = document.createElement('img');
      try {
          icon.src = `https://www.google.com/s2/favicons?domain=${child.url}&sz=128`;
   }  
      catch (e) {
  // 如果链接无效，显示默认图标
...
// 容错处理
      ...
      urlText.className = 'item-url';
      	try {
  		urlText.textContent = new URL(child.url).hostname;
	} catch (e) {
...
// --- 5. 加载书签数据并初始化侧边栏 ---
  chrome.bookmarks.getTree(function (nodes) {
    ...
    if (targetItem) {
        targetItem.click(); // 执行渲染[cite: 10]
        
        // 关键修复：延迟执行滚动计算[cite: 10]
        setTimeout(() => {
            const container = document.getElementById('sidebar-content'); //[cite: 10]
            if (container && targetItem) {
                // 计算偏移量：减去 40 是为了抵消 CSS 中的 border-top[cite: 9]
                const targetTop = targetItem.offsetTop - 40; 
                container.scrollTop = targetTop - (container.clientHeight / 2) + (targetItem.clientHeight / 2); //[cite: 10]
            }
        }, 150); 
    }
}); // 确保这里有分号收尾
